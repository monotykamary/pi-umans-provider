 ...
 * anthropic-messages API. Base URL: https://api.code.umans.ai
 *
 ...
 *
 * Key API details:
 *   - Uses Anthropic Messages-compatible endpoint
 ...
 *   - pi-ai's transform-messages.js handles orphaned tool_use blocks automatically
 *
...
const MODELS_INFO_URL = `${BASE_URL}/v1/models/info`;
const USAGE_API_URL = `${BASE_URL}/v1/usage`;
...
export default function (pi: ExtensionAPI) {
  ...
  // for orphaned tool_use blocks before the provider request is sent
  // (see AnthropicMessagesCompat and transform-messages.js). No manual repair needed.
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    cachedApiKey = (await ctx.modelRegistry.getApiKeyForProvider("umans").catch(() => undefined)) ?? undefined;

    // Background revalidate: fetch live models, merge, cache, hot-swap provider
