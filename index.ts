 ...
 *   - GLM 5.1 vision is server-side handoff — not available via raw API
...
export default function (pi: ExtensionAPI) {
  ...
  // ─── before_provider_request: strip reasoning_effort + sanitize orphaned tool_calls ───
  ...
  pi.on("before_provider_request", (event) => {
    ...
    console.warn(
      `[pi-umans-provider] Found ${orphanedIds.length} orphaned tool_call(s) without tool result: ${orphanedIds.join(", ")}`,
    );
